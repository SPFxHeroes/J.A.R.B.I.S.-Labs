...
## Exercise 1: Add an elements.xml file for SharePoint definitions
...
For more detail on the Feature element manifest schema, see: [Using Features in SharePoint Foundation](https://learn.microsoft.com/previous-versions/office/developer/sharepoint-2010/ms460318(v=office.14)?redirectedfrom=MSDN).

## Exercise 2: Add a schema.xml file for defining list structure
...
## Exercise 3: Add definitions to build pipeline
...
1. Open `package-solution.json` from the `config` folder.
    ...
    > The `package-solution.json` file defines the package metadata

1. Ensure the new Feature Framework files included in the SharePoint Package. To do so, find the `feature` node and add the following code after the `version` node (don't forget to add a `,` after the `version` line)
...
## Exercise 4: Deploy and test asset provisioning

Now you're ready to deploy the solution to SharePoint. Because we're provisioning assets directly to the SharePoint sites when the solution is installed, you can't test the capability in the Workbench.

1. In the terminal, execute the following command to package your client-side solution that contains the web part to get the basic structure ready for packaging:
...
1. Execute the following command to create the solution package:
    ...
    > The command creates the `jarbis.sppkg` package in the `sharepoint/solution` folder.

    > **NOTE:** You will see a warning message indicating that the solution is not a production build, and another one indicating that Feature.xml elements are not automatically applied unless the solution is explicitly installed on a site; both warnings are expected.

1. Deploy the package that was generated to the app catalog. In the browser, navigate to your tenant's app catalog by visiting https://_YOURTENANT_.sharepoint.com/sites/appcatalog/_layouts/15/tenantAppCatalog.aspx/manageApps -- where `YOURTENANT` is your Microsoft 365 developer tenant name.
1. Find the `jarbis.sppkg` package in the `sharepoint/solution` using File Explorer (Tip: right-click on the file from within VSCode and select "
...
1. When prompted to **Enable app**, select **Only enable this app**; this will ensure that we only deploy the solution to our test site.
   ...
   >
   > ![This app gets data from https://localhost:4321/dist/](assets/localresource.png)  
