...
We want our web part to read data from a list. We're going to use this list to help us generate our heroes.
...
If you skipped the previous step, or just want to start here, you can find the code ready to go in the [Lab 08 Starter](https://github.com/SPFxHeroes/JARBIS/tree/Start-of-Lab-08) branch.
...
This is done by using a Feature. A feature is a container that includes one or more SharePoint items to provision. A feature is made up of a **Feature.xml** file and one or more element manifest files. You can even include files that handle upgrades to previously provisioned assets. Wowee!
...
1. In the **assets** folder, create a new file called **elements.xml**
    ...
    ![elements.xml in explorer](./assets/elementsintree.png)
...
- Definitions use standard Feature Framework schema, which is "well known" to SharePoint developers. (This is some old school SharePoint stuff)
- Custom fields are being referenced in the introduced content type.
- We use the `CustomSchema` attribute in the `ListInstance` element to define a provisioning-time `schema.xml` file for the list (**line 271**). This way the list is still based on the out-of-the-box list template (normal custom list template `100` in this case), but we can define an alternative provisioning definition during initial provisioning.
...
1. From the **My apps** page, under **Apps you can add**, find **JARBIS - Hero Generator** and select **Add**.
